/**
 * colors.ts — deterministic colour helpers shared across routes.
 *
 * `segmentColor` maps any label string to a stable colour from a fixed palette
 * via a simple integer hash so the same agent/model/category name always gets the
 * same colour across renders and sessions.
 *
 ...
 *
 * Originally scoped to `app/agent-performance/_components/` — promoted to
 * `src/lib/` when CategoryDonut (StatsTab) needed the same helpers, to avoid
 * a cross-route private-folder import.
 */
